...
//				"apache.Fraction"
	...
	private String executeTest(Test orig) throws TestFailedException, Exception {
		...
		if(res.size() == 1) partsCov.add(combinedCov = getCoverage(res.get(0)));
			...
			for(Test r : res) {
				Context<ElementManager<String, CoverageInformation>, CoverageExecutionManager> ctx = CoverageExecutionManager.getContext(getFinder(), r, new BehaviorTrackerData(orig.getCluster().getXmlClasses()));
