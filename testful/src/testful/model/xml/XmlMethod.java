...
public class XmlMethod implements Comparable<XmlMethod> {
	...
	public static XmlMethod create(Method meth) {
		...
		for (Class<?> params : meth.getParameterTypes()) {
			// skipping arrays (supported via adapter methods)
			if(params.isArray()) return null;

			// ISSUE #1: if you need support for enums, vote here: http://code.google.com/p/testful/issues/detail?id=1
			if(params.isEnum()) {
		...
		if(Modifier.isStatic(meth.getModifiers())) xmeth.setKind(Kind.STATIC);
			...
			if(methName.startsWith("get") && methParam.length == 0) {
				...
				// which return a primitive type
				// and have not been generated by testful (e.g., to add array support)

				if(meth.getReturnType().isPrimitive() &&
						!meth.getDeclaringClass().getName().startsWith("testful.")) xmeth.setKind(Kind.OBSERVER);
