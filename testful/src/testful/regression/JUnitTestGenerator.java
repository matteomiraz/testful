...
public class JUnitTestGenerator extends TestReader {
	...
	public void read(String fileName, Test test) {
		TestCase testCase = suite.get(test.getCluster());
	...
	private class TestSuite extends ElementManager<TestCluster, TestCase>{
		private static final long serialVersionUID = -209417855781416430L;

		@Override
		public TestCase get(TestCluster cluster) {
			TestCase test = super.get(cluster);
			if(test == null) {
				test = new TestCase(cluster);
				put(test);
			}
			return test;
		}
	...
	private class TestCase implements ElementWithKey<TestCluster> {
		...
		public TestCase(TestCluster cluster) {
			final String fqClassName = cluster.getCut().getClassName();
			...
			AnalysisWhiteBox whiteAnalysis = AnalysisWhiteBox.read(config.getDirInstrumented(), fqClassName);
			data = Utils.readData(cluster.getXmlClasses(), whiteAnalysis);
		}
