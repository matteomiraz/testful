...
import static testful.utils.JavaUtils.getIndex;
	...
	public void internalTransform(Body body, String phaseName, Map options) {
		...
		for(int i = 0; i < units.length; i++) {
			...
			if(units[i] instanceof GotoStmt) {
				...
				System.out.println("if " + ((IfStmt) units[i]).getCondition() + " goto " + getIndex(units, ((IfStmt) units[i]).getTarget()));
				...
				for(int t = 0; t <= sw.getHighIndex() - lowIndex; t++)
					System.out.println("      case " + (t + lowIndex) + ": goto " + getIndex(units, sw.getTarget(t)));
				...
				for(int v = 0; v < sw.getTargetCount(); v++)
					System.out.println("      case " + sw.getLookupValue(v) + ": goto " + getIndex(units, sw.getTarget(v)));
				System.out.println("      default: goto " + getIndex(units, sw.getDefaultTarget()));
		...
		for(Trap t : body.getTraps())
			System.out.println(" [" + getIndex(units, t.getBeginUnit()) + "-" + getIndex(units, t.getEndUnit()) + ") -> (" + t.getException() + ") -> " + getIndex(units, t.getHandlerUnit()));
