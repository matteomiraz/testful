...
public class BehaviorTrackerData implements TrackerDatum {
	...
	public BehaviorTrackerData(Map<String,XmlClass> xmlClasses) {
		abstractorClass = new HashMap<String, AbstractorObjectState>();
		abstractorMethod = new HashMap<String, AbstractorMethod>();

		for(Entry<String, XmlClass> e : xmlClasses.entrySet()) {

			{ // create state abstractor
				testful.model.xml.behavior.Behavior beh = getBehavior(e.getValue().getExtra());
				if(beh != null) {
					...
					try {
						abstractorClass.put(e.getKey(), new AbstractorObjectState(state.toArray(new Abstractor[state.size()])));
					} catch(Exception ex) {
			...
			{ // create constructor abstractor
				for(testful.model.xml.XmlConstructor xmlCns : e.getValue().getConstructors()) {
					// create state abstractor
			...
			{ // create method abstractor
				for(testful.model.xml.XmlMethod xmlMeth : e.getValue().getMethods()) {
					// create state abstractor
