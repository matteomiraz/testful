...
/*
 * SPEA2Evaluator.java Created: Wed Jun 26 11:20:32 PDT 2002 By: Robert Hubley,
 * Institute for Systems Biology (based on Evaluator.java by Sean Luke)
 */
 ...
 * evaluating thread. The evaluator is also responsible for calculating the
 * SPEA2Fitness function. This function depends on the entire population and so
...
public class ParallelSPEA2Evaluator extends SimpleParallelEvaluator {

	private static final long serialVersionUID = 5549743780427986455L;
	...
	public void evaluatePopulation(final EvolutionState state) {
		for(int i = 0; i < state.population.subpops.length; i++)
			if(!(state.population.subpops[i] instanceof SPEA2Subpopulation)) state.output.fatal("SPEA2Evaluator must only be used with a SPEA2Subpopulation!", null);
		...
		for(int x = 0; x < state.population.subpops.length; x++) {
			...
			for(int y = 0; y < inds.length; y++) {
				...
				for(Individual ind : inds)
					if(inds[y].fitness.betterThan(ind.fitness)) myStrength++;
			...
			for(int y = 0; y < inds.length; y++) {
				...
				for(int z = 0; z < inds.length; z++) {
					...
					// Raw fitness
					if(inds[z].fitness.betterThan(inds[y].fitness)) rawFitness += ((SPEA2MultiObjectiveFitness) inds[z].fitness).SPEA2Strength;

					// Set distances
					if(y == z) distances[y][z] = 0;
					else if(z > y) {
						distances[y][z] = ((SPEA2MultiObjectiveFitness) inds[y].fitness).calcDistance((SPEA2MultiObjectiveFitness) inds[z].fitness);
				...
				kthDistance = 1 / Math.pow(dist, ((SPEA2MultiObjectiveFitness) inds[0].fitness).multifitness.length) * kTH / inds.length / vol;
				sumKthDistance += kthDistance;

				// Set SPEA2 raw fitness value for each individual
				((SPEA2MultiObjectiveFitness) state.population.subpops[x].individuals[y].fitness).SPEA2RawFitness = rawFitness;

				// Set SPEA2 k-th NN distance value for each individual
				((SPEA2MultiObjectiveFitness) state.population.subpops[x].individuals[y].fitness).SPEA2kthNNDistance = kthDistance;

			} // For each individual y

			// Normalize the kthDistance value
			for(int y = 0; y < inds.length; y++) {
				((SPEA2MultiObjectiveFitness) state.population.subpops[x].individuals[y].fitness).SPEA2kthNNDistance /= sumKthDistance;

				// Set SPEA2 fitness value for each individual
				((SPEA2MultiObjectiveFitness) state.population.subpops[x].individuals[y].fitness).SPEA2Fitness = ((SPEA2MultiObjectiveFitness) state.population.subpops[x].individuals[y].fitness).SPEA2RawFitness
				+ ((SPEA2MultiObjectiveFitness) state.population.subpops[x].individuals[y].fitness).SPEA2kthNNDistance;
	...
	/**
	 * Private helper function. This calculates the area of a sphere in n
	 * dimensions for use in the SPEA2 density calculation.
	 */
