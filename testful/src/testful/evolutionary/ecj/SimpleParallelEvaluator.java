...
/*
 * SimpleEvaluator.java Created: Wed Aug 18 21:31:18 1999 By: Sean Luke
 */
...
public class SimpleParallelEvaluator extends Evaluator {

	private static final long serialVersionUID = -7010321874148410309L;

	// checks to make sure that the Problem implements SimpleProblemForm
	@Override
	public void setup(final EvolutionState state, final Parameter base) {
		super.setup(state, base);
		if(!(p_problem instanceof SimpleProblemForm)) state.output.fatal("" + this.getClass() + " used, but the Problem is not of SimpleProblemForm", base.push(P_PROBLEM));
	}
	...
	public void evaluatePopulation(final EvolutionState state) {
		...
		for(int y = 0; y < state.evalthreads; y++)
			for(int x = 0; x < state.population.subpops.length; x++) {
				...
				if(y < state.evalthreads - 1) // not last one
					...
					(state.population.subpops[x].individuals.length - (state.population.subpops[x].individuals.length / state.evalthreads) // note integer division
							* state.evalthreads);
		...
		if(state.evalthreads == 1) evalPopChunk(state, numinds[0], from[0], 0, (SimpleProblemForm) (p_problem.clone()));
	 ...
	 * declared public (for the benefit of a private helper class in this file),
	...
	public boolean runComplete(final EvolutionState state) {
		for(Subpopulation subpop : state.population.subpops)
			for(Individual individual : subpop.individuals)
				if(individual.fitness.isIdealFitness()) return true;
		return false;
	}
}
...
class SimpleEvaluatorThread implements Runnable {
	...
	public int[] numinds;
	public int[] from;
	public SimpleParallelEvaluator me;
